...
function getInputValueNext(e){
  if (document.getElementById("input-string").value == '') {
    ...
    localStorage.setItem('paraprash_content', document.getElementById("input-string").value.replace(/\/$/, ""));
...
function active_buttonkk(active_button){
  ...
  input1 = document.getElementById("paraphrase-output-1").value
  ...
  // document.getElementById("paraphrase-output-1").addEventListener("keyup",()=>{
  ...
  function paraprashstyle(){
    characterCount2.innerHTML = document.getElementById("paraphrase-output-1").value.length;
    var words = document.getElementById("paraphrase-output-1").value.match(/\b[-?(\w+)?]+\b/gi);
    if (words) {
      wordCount2.innerHTML = words.length;
    const wordsPerMinute = 200; // Average case.
    const wordsSpeakMinute = 230; // Average case.
