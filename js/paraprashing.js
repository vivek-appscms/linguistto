  ...
  async function showParaphrase(start, end, option) {
    for (let i = start; i < end; i++) {
      try {
        ...
        try {
          const api_url =  `/.netlify/functions/paraphrase/?baselocalecode=${lang[i]}&datalanguagecode=${
            lang[i + 1]
          }&value=${paraphrase}`
          const response = await fetch(api_url);
          let data = await response.json();
...
//       } catch (error) {
//         try {
//           const api_url =  `http://127.0.0.1:9000/paraphrasing/?baselocalecode=${lang}&datalanguagecode=${
//             lang[i + 1]
//           }&value=${paraphrase}`
//           return await getapi(api_url)
//         } catch (error) {
